...
| H-7 Fix B deployed; actor-binding/reactivation follow-up migration + Edge source still local/undeployed | `backend/supabase/migrations/README.md`, `DECISIONS.md` 2026-09-09 |
...
| H-7 | Final RLS / cross-shop isolation audit | CODE COMPLETE · AUTOMATED + SINGLE-DEVICE PHYSICAL PASS · TWO-DEVICE CONVERGENCE NOT RUN | CRITICAL — cross-shop leakage | H-7 migrations through `20260909000000_h7_actor_binding_staff_reactivation.sql`, SQLite `0027`-`0029`, `functions/sync/`, `apps/mobile/sync/` | **migration 26 + matching `sync` redeploy still pending** | stale-JWT shared-device repro PASS; Owner-only reactivation PASS; single-device H-7 matrix PASS; no known application defect. Two-device convergence **NOT RUN** (one device only) — deferred to post-RC/pilot, must not be recorded as verified |
