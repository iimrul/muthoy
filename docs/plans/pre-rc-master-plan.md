...
| SQLite migrations `0000`-`0029` registered in runtime order | `apps/mobile/db/migrations/` |
| Hosted ledger 25/25 through `20260907020000_h7_fix_pass_b.sql`; `sync` v14 ACTIVE (2026-09-07 baseline, superseded below) | `backend/supabase/migrations/README.md` |
| H-7 actor-binding/reactivation follow-up deployed 2026-09-10: hosted ledger 26/26, `sync` v16 ACTIVE — confirmed by direct hosted verification 2026-09-12 (`supabase migration list` + `functions list`) | `backend/supabase/migrations/README.md`, `DECISIONS.md` 2026-09-12 (correction entry) |
| H-7 automated + single-device physical validation PASS, incl. the `0027`-`0029` on-device upgrade; no known H-7 application defect | `DECISIONS.md` 2026-09-12 |
| H-7 physical two-device convergence **NOT RUN** — no second device; deferred to post-RC/pilot, not verified | `DECISIONS.md` 2026-09-12 |
| Recorded suite: 170 files / 2,058 tests + typecheck + lint PASS | `DECISIONS.md` 2026-09-12 |
...
| H-4 | PIN timing / security hardening | **SIGNED OFF 2026-09-22** | was HIGH — timing oracle on PIN verify; open latency gate | `db/auth.ts`, `native/crypto.ts`, `dev/authTiming.ts`, `db/pin-performance.sqlite.test.ts` | possible native rebuild | **PASS on SM-A047F.** Fixed-work compare + quantised response (`db/pinTiming.ts`); offline attempt budget (`db/pinAttemptLock.ts`) locks on the 5th miss with canonical state, survives force-stop, and a valid **Staff** PIN no longer refills the shop-wide budget. Owner recovery reachable during an active lock. Offline PIN login min 827 ms / median 1176 ms / max 1676 ms — inside the §16 gate |
...
| H-7 | Final RLS / cross-shop isolation audit | CODE COMPLETE · AUTOMATED + SINGLE-DEVICE PHYSICAL PASS · TWO-DEVICE CONVERGENCE NOT RUN | CRITICAL — cross-shop leakage | H-7 migrations through `20260909000000_h7_actor_binding_staff_reactivation.sql`, SQLite `0027`-`0029`, `functions/sync/`, `apps/mobile/sync/` | **migration 26 + matching `sync` redeploy: DONE 2026-09-10 (26/26, `sync` v16) — confirmed 2026-09-12** | stale-JWT shared-device repro PASS; Owner-only reactivation PASS; single-device H-7 matrix PASS; no known application defect. Two-device convergence **NOT RUN** (one device only) — deferred to post-RC/pilot, must not be recorded as verified |
...
| H-10 | Auth / device-link / session hardening | **A2.1 / A2.2 / A2.4 SIGNED OFF 2026-09-22** · **A2.3 device registry NOT STARTED** | HIGH | `sync/sessionAuthority.ts`, `sync/sessionAuthorityStore.ts`, `sync/authMutation.ts`, `state/signOutDevice.ts`, `app/_layout.tsx`, `sync/deviceAuth.ts`, `sync/authClaims.ts` | none | **PASS on device.** 7-day authority lease with trusted high-water anchor; pre-navigation gate holds the authenticated tree closed until confirmation; server-side staff deactivation quarantines that actor on reconnect, the quarantine survives restart and a correct local PIN cannot bypass it; recovery requires authoritative credential proof, not merely re-enabling the account; a deliberate network drop writes **no** quarantine and reconnect reconfirms. **H-10 is not closed until A2.3 ships** |
| H-11 | Migration / rollback / recovery readiness | **Documentation + tooling SIGNED OFF 2026-09-22** · **C5 rehearsal STILL OWED** | CRITICAL — recovery still unrehearsed against a real backup provider | `backend/supabase/migrations/ROLLBACK.md`, `backend/supabase/migrations/rollback-classification.json`, `scripts/classify-migrations.mjs`, `scripts/render-rollback-docs.mjs`, `backend/supabase/checks/restore_baseline.sql`, `backend/supabase/checks/restore_drill.md` | rehearsal only | Classification corrected to **A 6 · B 13 · C 7 of 26** from a single source of truth, generated into three documents and drift-guarded by test. 16-table restore fingerprint (ordered `md5` + summed paisa) with a corrupt → detect → restore cycle executing in PGlite — automated SQL validation only. **Every row of `restore_drill.md` still reads `_not run_`** |
...
6. H-4 PIN timing/security hardening + close the latency gate — **SIGNED OFF 2026-09-22**.
...
9. H-10 auth/device-link/session hardening — A2.1/A2.2/A2.4 **SIGNED OFF 2026-09-22**;
   **A2.3 device registry is the next Wave 2 sub-batch** and is not started.
10. H-11 migration/rollback/recovery — documentation and tooling **SIGNED OFF
    2026-09-22**; the **C5 operational rehearsal is still owed**.
...
**SIGNED OFF 2026-09-22** together with an end-to-end sale/stock/cash regression
(stock 100 → 99 exact; ৳10.00 − ৳2.00 discount = ৳8.00; drawer ৳1,008.00 =
৳1,000 opening + ৳8 cash). Carry-forward defects **D8/D9** (sale-complete and
Sales History are hardcoded English), **D10** (inventory list refresh) and
**D11** (language-toggle hit area) are recorded in `DECISIONS.md`.
