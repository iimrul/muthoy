...
const deps = vi.hoisted(() => ({
  ...
  triggerSyncAfterInteractions: vi.fn((_shopId: string, onStart?: () => void) => {
    onStart?.();
    return vi.fn();
  }),
}));
...
const { default: StaffManagementScreen } = await import('../app/staff/management');
...
async function reachPinSetup(): Promise<void> {
  ...
  fireEvent.change(screen.getByLabelText('Staff name'), { target: { value: 'Arif' } });
  fireEvent.change(screen.getByLabelText('Staff phone number'), { target: { value: '01712345678' } });
  fireEvent.click(screen.getByRole('button', { name: 'Next' }));
  fireEvent.click(screen.getByRole('button', { name: 'Next' }));
  expect(screen.getByText('Set a PIN for Arif')).toBeTruthy();
}
...
describe('staff PIN creation', () => {
  it('submits the confirmed four-digit PIN once and creates the staff login', async () => {
    ...
    expect(deps.createStaff).toHaveBeenCalledWith(
      SHOP_ID,
      OWNER_ID,
      expect.objectContaining({ name: 'Arif', phone: '01712345678', rawPin: '4321' }),
      expect.any(Function),
      expect.objectContaining({ flow: 'staff_creation' }),
    );
    expect(deps.triggerSyncAfterInteractions).toHaveBeenCalledWith(SHOP_ID, expect.any(Function));
  ...
  it('surfaces a PIN collision instead of the generic failure dialog', async () => {
    ...
    expect(await screen.findByText('That PIN is already used by someone at this shop. Choose another.')).toBeTruthy();
    expect(native.alert).not.toHaveBeenCalled();
    expect(consoleError).toHaveBeenCalledWith(
      '[staff:create] failed',
      expect.objectContaining({
        code: 'DuplicatePinError',
        message: 'That PIN is already used by someone at this shop. Choose another.',
        context: { operation: 'createStaff', shopId: SHOP_ID, actorUserId: OWNER_ID },
      }),
    );
  });
});
