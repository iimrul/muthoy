    ...
    <View className="flex-1 bg-brand-softGreen">
      ...
      <ScrollView contentContainerClassName="gap-4 p-4 pb-28">
        ...
        {totals && hasActivity ? <>
          ...
          <View className="flex-row gap-2">{[{ key: 'print' as const, icon: 'printer' as const, label: bn ? 'প্রিন্ট' : 'Print', run: print }, { key: 'csv' as const, icon: 'download' as const, label: 'CSV', run: () => exportReport('csv') }, { key: 'xlsx' as const, icon: 'share-2' as const, label: 'Excel', run: () => exportReport('xlsx') }].map((item) => <Pressable key={item.key} disabled={action !== null} onPress={() => void item.run()} className="flex-1 items-center gap-1 rounded-xl bg-white p-3 disabled:opacity-50"><View className="h-9 w-9 items-center justify-center rounded-lg bg-brand-softGreen"><Feather name={item.icon} size={16} color="#059669" /></View><Text className="font-sans-bold text-xs">{action === item.key ? '…' : item.label}</Text></Pressable>)}</View>
        </> : null}
      </ScrollView>
    </View>
  );
