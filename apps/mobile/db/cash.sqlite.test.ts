...
const { batches, credits, customers, medicines, roles, saleItems, sales, shops, users } = schema;
const {
  closeDay,
  currentBusinessDate,
  getCashSummarySync,
...
describe('expense recording and its cash impact', () => {
  ...
  it('rejects a non-positive amount without writing anything', async () => {
    ...
    await expect(
      recordExpense({ shopId: fixture.shopId, staffId: fixture.ownerId, category: 'rent', amount: asPaisa(0) }),
  ...
  it('refuses to record an expense into an already-closed day', async () => {
    ...
    await expect(
      recordExpense({ shopId: fixture.shopId, staffId: fixture.ownerId, category: 'supplies', amount: asPaisa(500) }),
