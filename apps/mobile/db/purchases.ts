...
import { purchaseHasPayment } from './suppliers';
...
export async function listPurchasesForSupplier(
  ...
  return rows.map((row) => ({ ...row, total: asPaisa(row.total), paidAmount: asPaisa(row.paidAmount) }));
}
...
export async function createPurchase(
  ...
  // Credit-term contribution to supplier payable (review §2 clarification):
  // COD contributes 0 (paidAmount === total from creation, via the synthetic
  // supplier_payment below); credit-terms contributes `total - paidAmount`
  // until settled or voided; a voided purchase always contributes 0
  // (guaranteed by voidPurchase's zero-movements/zero-payments invariant).
  ...
  return db.transaction((tx) => {
    ...
    const codDrawer = input.paymentType === 'cod'
    ...
    let expectedCount = 1; // purchases insert
...
export async function markPurchaseLineReceived(
  ...
  return db.transaction((tx) => {
    ...
    // COD atomic settlement (review §3 fix): a COD purchase's synthetic
    // full-payment at creation is sized only from non-pending lines, so
    // receiving a pending line later must pay the newly-received delta in
    // THIS same transaction — otherwise a COD invoice would go "unpaid"
    // after receive, breaking the COD-always-fully-paid invariant. Sized
    // here, before expectedCount, so the group's row count is exact.
    const isCod = purchase.paymentTerms === 'cod';
    const codDrawer = isCod
      ? tx.select({ id: cashDrawer.id, isDeleted: cashDrawer.isDeleted })
          .from(cashDrawer)
    ...
    const codOperationCount = isCod ? 1 + (codDrawer ? 1 : 2) : 0;
    ...
    // COD stays fully paid: paidAmount tracks the recomputed total in the
    // SAME update as `total` (no extra purchases row/op), and the delta this
    // line contributed is settled via a synthetic supplier_payment + drawer
    // write below, mirroring createPurchase's COD branch and
    // recordSupplierPayment's cash path.
    const purchaseValues = isCod
      ? stampUpdatedAt({ total: newTotal, paidAmount: newTotal, isDirty: true })
    ...
    if (isCod) {
      ...
      const codPaymentValues = {
        ...
        partyId: purchase.supplierId, amount: lineValue, method: 'cash' as const,
...
interface RawPurchaseLineRow extends Omit<PurchaseDetailLine, 'purchasePrice'> {
...
export async function getPurchaseDetail(
  ...
  return {
    ...
    lines: lineRows.map((line) => ({ ...line, purchasePrice: asPaisa(line.purchasePrice) })),
  };
}
