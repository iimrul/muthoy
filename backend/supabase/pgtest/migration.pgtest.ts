...
const B3_GROUP2_SYNC_MIGRATION = "20260823020000_b3_group2_sync_completion.sql";
 ...
 */
function everyTablePayload(): Array<{
  table: string;
  ...
  return [
    ...
    {
      table: "suppliers",
      row: { ...base, id: IDS.supplier, shop_id: SHOP_A, name: "Beximco" },
    },
    {
      table: "purchases",
      row: {
    ...
    },
    {
      ...
      row: {
        ...
        shop_id: SHOP_A,
        purchase_id: IDS.purchase,
        purchase_item_id: IDS.purchaseItem,
        qty: 1,
        credit_amount: 1000,
        created_by: OWNER_A,
      },
...
describe("the migration applies to a real Postgres", () => {
  ...
  it("re-applies without turning the wrapper into an infinite recursion", async () => {
    ...
    try {
      ...
      await h.exec(migrationSql(HARDENING_MIGRATION));
      await h.exec(migrationSql(B3_GROUP2_SYNC_MIGRATION));
      await h.exec(migrationSql(B3_GROUP2_SYNC_MIGRATION));
      ...
      const bases = await h.all(
        `select 1 from pg_proc where proname = 'sync_apply_row_base'`,
      );
      expect(bases).toHaveLength(1);
      expect(
        await h.all(
          `select 1 from pg_proc where proname='sync_apply_row_base_pre_b3_cash_reconcile'`,
        ),
      ).toHaveLength(1);
      expect(
        await h.all(
          `select 1 from pg_proc where proname='sync_apply_operation_pre_b3_withdrawal'`,
        ),
      ).toHaveLength(1);
...
describe("a default staff member can still complete a checkout", () => {
  ...
  it("and the ledger moved the stock the sale removed", async () => {
    const batch = await h.one<{ stock: number }>(
      `select stock from batches where id = $1`,
      [IDS.batch],
    );
